<template>
  <div v-show="false">
    ...
    <div class='butn-div'>
      ...
      <div onclick="AppBus(${windowCYQ})" style="background: url(../../static/images/home/btn_caiyq_normal.png) no-repeat center">
      </div>
    </div>
  </div>
</template>
...
export default {
  ...
  mounted: function() {
    ...
    bus.$on("PFKFJ", function(data) {
      //_self.BindData(data);
      _self.EditData(sitewhere.DevicesGroupData[_self.token]);
    });
    bus.$on("PFKFJ_Refresh", function(data) {
      _self.EditData(data[_self.token]);
    });
    bus.$on("SearchRefresh", function(data) {
      if (data.TabPage == "pfkfjgrid" || data.TabPage == "") {
